      ...
      <Card style={[styles.card, isInConsultation && styles.activeCard]}>
        <View style={styles.header}>
          <Typography variant="bodyLg" color="neutral.900" style={{ fontWeight: 'bold' }}>
            {formatTime(item.time)} - {item.users?.name || item.walk_ins?.name || 'Unknown Patient'}
          </Typography>
          <StatusPill status={item.status} />
        </View>

        <Typography variant="bodyMd" color="neutral.700" style={{ marginBottom: 12 }}>
          Queue #{item.queue_order} | Phone: {item.users?.phone || item.walk_ins?.phone || 'N/A'}
        </Typography>
        ...
        <View style={styles.actions}>
          <Button
            ...
            onPress={() => navigation.navigate('PatientDetails', {
              patientId: item.users?.id || item.walk_ins?.id,
              appointmentId: item.id,
              isWalkIn: !!item.walk_ins
            })}
          />
